...
This is an async multi-agent demo that connects an LLM client to one or more MCP (Modular Connector Protocol) tool servers. It demonstrates using Assistant agents backed by different MCP toolsets (HTTP/SSE and stdio-based adapters) and a UserProxy agent, orchestrated via a SelectorGroupChat with streaming Console output. 🚀
